...
class PeoplePerceptorBase(Perceptor):
    ...
    def __find_face_rectangle(self, body, config):
        ...
        for label, keypoint in body["pose"].keypoints.items():
            ...
            iter += 1
            if iter < 5:
                if int(keypoint.point[0]) < lowestX:
                    lowestX = int(keypoint.point[0])
                if int(keypoint.point[1]) < lowestY:
                    lowestY = int(keypoint.point[1])
                if int(keypoint.point[0]) > highestX:
                    highestX = int(keypoint.point[0])
                if int(keypoint.point[1]) > highestY:
                    highestY = int(keypoint.point[1])
            else:
                continue
