...
class TestEventEmitter:
    ...
    def test_on_adds_listener(self):
        emitter = EventEmitter()
        emitter.__event_names = ["event_1"]
        emitter.on("event_1", lambda: None)
        assert len(emitter.__event_handlers["event_1"]) == 1
    ...
    def test_on_adds_multiple_listeners(self):
        ...
        emitter.on("event_1", lambda: None)
        emitter.on("event_1", lambda: None)
        assert len(emitter.__event_handlers["event_1"]) == 2
    ...
    def test_off_removes_listeners(self):
        ...
        emitter.on("event_1", lambda: None)
        emitter.off("event_1")
        assert "event_1" not in emitter.__event_handlers

    def test_get_event_names(self):
        emitter = EventEmitter()
        event_names = ["event_1", "event_2"]
        emitter.__event_names = event_names
