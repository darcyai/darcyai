...
def people_input_callback(input_data, pom, config):
    # Just take the frame from the incoming Input Stream and send it onward - no need to modify
    # the frame
...
def live_feed_callback(pom, input_data):
    """
    Callback function for the Live Feed output stream.
    """
    ...
    label = f"{pom.peeps.peopleCount()} peeps"
    color = (0, 255, 0)
    ...
    if pom.peeps.peopleCount() > 0:
        for person_id in pom.peeps.people():
            ...
            label2 = f"{face_height} face height"
            color = (0, 255, 255)
...
# Subscribe to the "New Person" event from the People Perceptor and use our callback from above as
# the handler
people_ai.on("new_person_entered_scene", new_person_callback)

# Add the People Perceptor instance to the Pipeline and use the input callback from above as the
# input preparation handler
pipeline.add_perceptor("peeps", people_ai, input_callback=people_input_callback)

# Update the configuration of the People Perceptor to show the pose landmark dots on the
