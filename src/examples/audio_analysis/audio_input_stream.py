...
class AudioInputStream(InputStream):
    ...
    def __init__(self,
        ...
        self.__input_device_index = self.__pyaudio.get_default_input_device_info()[
            "index"]
        self.__channels = self.__pyaudio.get_default_input_device_info()[
            "maxInputChannels"]
    ...
    def stream(self):
        ...
        while not self.__stopped:
            ...
            for i in range(0, int(self.__rate / self.__chunk_size * self.__sample_len_sec)):
                data = self.__audio_stream.read(
                    self.__chunk_size, exception_on_overflow=False)
                frames.append(data)
