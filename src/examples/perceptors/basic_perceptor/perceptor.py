...
# Define a class called "BasicPOM" which is serializable which is a Perception Object Model (POM)
# compatible output
# The Darcy AI engine will add this POM-ready output to the POM which moves along the pipeline
# and to the output streams
...
class BasicPerceptor(Perceptor):
    """
    This is a basic Perceptor which simply returns value of the counter.
    """
    # Define our "init" method
    def __init__(self, sleep=0.05):
        # Call "init" on the parent class but don't pass an AI model path because we are just
        # hard-coding our demo output
    ...
    def run(self, input_data, config):
        """
        This method is called by the Darcy AI engine to run the perceptor.
        """
        # Simply sleep for the configured amount of time
        time.sleep(self.__sleep)
        self.__counter += 1

        # Produce a simple text string output with the counter value so a consumer of this demo
        # can see it is going up
        result = f"Hello World #{self.__counter}"

        # Emit the event called "event_1" so a consumer of this demo can subscribe to this type
        # of event and see that it is working
        self.emit("event_1", result)

        # Wrap the output data in the BasicPOM type and send it out
        return BasicPOM(result)

    # Define our "load" method which does not need to do anything except set the value on the
    # parent class
    # pylint: disable=unused-argument
