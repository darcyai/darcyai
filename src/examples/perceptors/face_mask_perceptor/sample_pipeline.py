...
class SamplePipeline():
    """
    This class is used to run the sample pipeline.
    """
    def __init__(self):
        # Use the CameraStream input stream
        camera = CameraStream(video_device="/dev/video0", fps=20)

        # Set up a Flask app so we can host a live video stream viewing UI and a configuration
        # REST API
    ...
    # Define our "run" method which just calls "run" on the pipeline but also starts our Flask app
    # on port 8080 for the REST API
    def run(self):
        threading.Thread(
    ...
    # Take the incoming data which is a video frame and add information about detected or not
    # detected face masks
    def __live_feed_callback(self, pom, input_data):
        """
        Callback for live feed output stream.
        """
        frame = input_data.data.copy()

        if pom.face_mask.has_mask():
    ...
    # We just need to crop the frame down to just the center and change the color order of the
    # video frame because our AI model wants RGB instead of BGR
