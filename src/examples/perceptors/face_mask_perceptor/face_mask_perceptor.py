...
class FaceMaskPerceptor(ImageClassificationPerceptor):
    ...
    def __init__(self, threshold: float = 0.95):
        ...
        coral_model_file = os.path.join(
            script_dir, "face_mask_detection_coral.tflite")
        cpu_model_file = os.path.join(
            script_dir, "face_mask_detection_cpu.tflite")
        ...
        if len(perception_result) > 0:
            try:
                idx = [i for i, x in enumerate(
                    perception_result) if x.name == 'Mask']
                if len(idx) > 0:
                    threshold = self.get_config_value("threshold") / 100
                    has_mask = bool(
                        perception_result[idx[0]].confidence >= threshold)
            except:
                has_mask = False
