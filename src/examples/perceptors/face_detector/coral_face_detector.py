...
# Add Darcy AI libraries we need, particularly the ObjectDetectionPerceptor base class and the
# Config and ConfigRegistry classes
...
class CoralFaceDetector(ObjectDetectionPerceptor):
    ...
    def __init__(self, threshold: float = 0.95):
        ...
        model_file = os.path.join(script_dir,
        ...
        # Add a configuration item to the list, in this case a threshold setting that is a
    ...
    def run(self, input_data: Any, config: ConfigRegistry = None) -> Any:
        ...
        """
        # Get the AI model result by calling "run" on the parent class where we have already
        # passed our AI model
        perception_result = super().run(input_data=input_data, config=config)

        # Create an empty result array and then fill it by checking all results against the
