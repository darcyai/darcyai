...
def live_feed_callback(pom, input_data):
    """
    Callback for the live feed stream.
    """
    frame = input_data.data.copy()
    if pom.object_detection is None:
        return frame

    for obj in pom.object_detection:
        cv2.rectangle(frame,
                      (obj.xmin, obj.ymin),
                      (obj.xmax, obj.ymax),
                      (0, 255, 0),
...
coral_model_file = os.path.join(script_dir,
                                "models",
                                "coral_ssd_mobilenet_v2_coco_quant_postprocess_edgetpu.tflite")
...
pipeline.add_perceptor("object_detection",
                       object_detection,
                       accelerator_idx=0,
                       input_callback=perceptor_input_callback)
